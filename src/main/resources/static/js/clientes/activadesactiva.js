...
app.controller('CorreriaController', function($scope,$http) { 
	...
	$scope.cargar = function(){
		var promesa = $http.get("/api/clientes?projection=ClienteVendedor&size=9999&sort=ciudad.ciudad,asc");
	...
	$scope.actualizar = function( cliente ){
		if( cliente.activo != "S" && cliente.activo != 'N'){
			alert("Opciones para activo S/N");
			return;
		}
		var promesa = $http.post( "/api/clientes/actualizar/activo", cliente );
		promesa.success( function( data , status , headers , config ) {
